...
def main():
    print("Fetching file size...", flush=True)
    try:
    ...
    try:
        ...
        print(f"Error fetching tail data: {e}")
        return
    ...
    if cd_offset == 0xFFFFFFFF or cd_size == 0xFFFFFFFF:
        ...
        if z64_idx != -1:
            ...
            try:
                ...
                print(f"Error fetching Zip64 EOCD: {e}")
                return
            rec_fmt = "<4sQHHIIQQQQ"
    ...
    for i in range(total_entries):
        ...
        # H (version needed) 2
        # H (flags) 2
        # H (compression method) 2
        # H (last mod time) 2
        # H (last mod date) 2
        # I (crc32) 4
        # I (compressed size) 4
        # I (uncompressed size) 4
        # H (filename length) 2
        # H (extra field length) 2
        # H (file comment length) 2
        # H (disk number start) 2
        # H (internal file attributes) 2
        # I (external file attributes) 4
        # I (relative offset of local header) 4

        # 4+2+2+2+2+2+2+4+4+4+2+2+2+2+2+4+4 = 46 bytes
        # struct format: "<4sHHHHHIIIHHHHHII"
        # sizes: 4 + 2*6 + 4*3 + 2*5 + 4*2
        # 4 + 12 + 12 + 10 + 8 = 46 bytes
        fmt = "<4sHHHHHIIIHHHHHII"
        unpacked = struct.unpack(fmt, cd_data[pos:pos+46])

        comp_method = unpacked[5]
