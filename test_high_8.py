...
for sign in [1, -1]:
    ...
    # Try high 8 bytes of X
    ks1 = b''.join([long_to_bytes(W[0], 16)[:8] for W in [W0, W1, W2, W3, W4, W5, W6, W7]])
    pt1 = bytes([c ^ k for c, k in zip(ct_full, ks1)])
    if all(32 <= b <= 126 for b in pt1[8:16]):
        print(f"Sign {sign} High 8 X PT:", pt1)

    # Try low 8 bytes of X
    ks2 = b''.join([long_to_bytes(W[0], 16)[8:] for W in [W0, W1, W2, W3, W4, W5, W6, W7]])
    pt2 = bytes([c ^ k for c, k in zip(ct_full, ks2)])
    if b'SK-CERT' in pt2:
        print(f"Sign {sign} Low 8 X PT:", pt2)
