...
for sign in [1, -1]:
    ...
    b_x0 = long_to_bytes(W0[0], 16)
    b_y0 = long_to_bytes(W0[1], 16)
    b_x1 = long_to_bytes(W1[0], 16)
    b_y1 = long_to_bytes(W1[1], 16)

    ks1 = bytes([b for pair in zip(b_x0, b_y0) for b in pair]) + bytes([b for pair in zip(b_x1, b_y1) for b in pair])
    pt1 = bytes([c ^ k for c, k in zip(ct_full, ks1)])
    if b'SK-CERT' in pt1: print("Found Interleaved!", pt1)
